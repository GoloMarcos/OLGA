...
def one_class_processing(data,normal_class:int):
  labels, train_idx, val_idx, test_idx = one_class_labeling(data, data.labels,normal_class)
  return one_class_masking(labels, train_idx, val_idx, test_idx)


def one_class_labeling(data, labels, normal_class:int):
	
  train_idx=np.where(data.train==1)[0]
  val_idx = np.where(data.val==1)[0]
  ...
  return labels.astype("bool"), train_idx, val_idx, test_idx

#训练集60%正常、验证集15%正常、测试集25%正常，验证集测试集中的正常异常样本1:1
def one_class_masking(labels, train_idx, val_idx, test_idx):
